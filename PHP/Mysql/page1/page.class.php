	...
	*/
	class Page {
		...
		public function __construct($total, $listRows=25, $query="", $ord=true){
			...
			if(!empty($_GET["page"])) {
				...
				if($page > $this->pageNum){
					echo "<script>alert('数据总数没有到达".$page."页,自动跳转到最后一页')</script>";
					$page = $this->pageNum;
				}
			}else{
				if($ord)
		...
		private function goPage(){
    			if($this->pageNum > 1) {
				return '&nbsp;<input style="margin-right:5px;width:50px;height:17px !important;height:18px;border:1px solid #CCCCCC;" type="text" onkeydown="javascript:if(event.keyCode==13){var page=(this.value>'.$this->pageNum.')?'.$this->pageNum.':this.value;location=\''.$this->uri.'page=\'+page+\'\'}" value="'.$this->page.'"><input style="cursor:pointer;width:30px;height:18px;border:1px solid #CCCCCC;" type="button" value="GO" onclick="javascript:var page=(this.previousSibling.value>'.$this->pageNum.')?'.$this->pageNum.':this.previousSibling.value;location=\''.$this->uri.'page=\'+page+\'\'">&nbsp;';
			}
		}
